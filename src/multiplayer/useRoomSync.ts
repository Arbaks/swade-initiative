...
function isGameState(value: unknown): value is GameState {
  ...
  return candidate.version === 1 && Array.isArray(candidate.participants) && !!candidate.deck
}

function roomFromSnapshot(value: unknown): RoomRecord | null {
  if (!value || typeof value !== 'object') return null
  const candidate = value as Partial<RoomRecord>
  if (typeof candidate.ownerId !== 'string' || !isGameState(candidate.state)) return null
  return {
